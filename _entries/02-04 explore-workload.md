...
Connect to PostgreSQL and create the empty database:
...
Flag reference:
- `--no-owner` — skip ownership assignment (avoids errors when the original owner doesn't exist)
- `--no-privileges` — skip privilege (GRANT/REVOKE) statements
- `--verbose` — print progress as each object is restored

> If your dump is a plain SQL file instead of a custom-format dump, use `psql` instead:
> ```sh
> psql -h <postgresql-fqdn> -U <pgadmin> -d orders_demo -f orders_demo.sql
> ```
