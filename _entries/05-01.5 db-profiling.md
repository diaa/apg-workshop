...
| `parallel_workers_launched` vs `_to_launch` | Close to equal | Large gap means `max_parallel_workers` or `max_parallel_workers_per_gather` is too low |
...
> **Note (PG 17+):** WAL write/sync counts and timing moved from `pg_stat_wal` to `pg_stat_io` (where `object = 'wal'`). The second query above shows those per-backend-type I/O stats. Enable `track_wal_io_timing = on` to populate the timing columns.
...
| `track_wal_io_timing` | `on` | Enables WAL write/sync timing in `pg_stat_io` (where `object = 'wal'`) |
...
| **WAL / Redo / TLog** | `pg_stat_wal`, `pg_stat_io` (object=wal) | `v$log`, `v$archived_log` | `dm_io_virtual_file_stats` (log file) |
